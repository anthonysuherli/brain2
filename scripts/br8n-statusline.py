...
Wiring: br8n's SessionStart hook (``hooks/first-run-init.py`` →
``ensure_global_statusline``) registers this in the user's *global* Claude
settings automatically on first run — the cue is cross-repo by design, so once
br8n is installed it shows in every repo. To wire it by hand (or pin it to one
repo) add to ``.claude/settings.json``:

    "statusLine": {
      "type": "command",
      "command": "python3 /absolute/path/to/br8n/scripts/br8n-statusline.py"
    }

Requires Python 3.11+: ``python3`` on the PATH must resolve to 3.11+. The macOS system
``python3`` is 3.9, whose ``datetime.fromisoformat`` rejects the variable-precision
timestamps PostgREST returns (``…04.8827`` → ValueError), which silently collapses the
cloud-tier line to NO_CAPTURE. ``parse_iso`` below tolerates that defensively, but 3.11+
is the supported target (the project requires ``>=3.11``).

Contract: read-only, fast, never fails — degrades gracefully at every level,
always exits 0. A statusline must never break the prompt. Pure stdlib only.
"""
...
def resolve_tier(env: dict) -> str:
    backend = os.environ.get("BR8N_BACKEND") or env.get("BR8N_BACKEND")
...
# ── local-tier source ───────────────────────────────────────────────────────
def db_path() -> str:
    return os.environ.get("BR8N_DB_PATH", os.path.expanduser("~/.br8n/brain.db"))
...
def _cache_path(base: str, project: str) -> str:
    ...
    return os.path.join(tempfile.gettempdir(), f"br8n-sl-{tag}.json")
...
def _portfolio_main() -> None:
    """Legacy cross-branch portfolio view. Activate with BR8N_STATUSLINE=portfolio."""
    cwd = os.getcwd()
    try:
