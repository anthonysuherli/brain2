...
def test_render_line1_with_hypothesis():
    line, _ = sl.render_line1("br8n", "dev", "fix the thing", width=80)
    line = _strip(line)
    assert "🧠 br8n" in line
    assert "▶ dev" in line
...
def test_render_line1_no_hypothesis():
    line, _ = sl.render_line1("br8n", "dev", None, width=80)
    line = _strip(line)
    assert "▶ dev" in line
...
def test_render_line1_truncates_hypothesis():
    long_hyp = "x" * 100
    line, hyp_fits = sl.render_line1("br8n", "dev", long_hyp, width=60)
    line = _strip(line)
    assert "…" in line
...
def _run_statusline(stdin_json: str, env_overrides: dict | None = None) -> tuple[int, str]:
    ...
    import sys as _sys
    script = str(_pathlib.Path(__file__).parent / "br8n-statusline.py")
...
def test_never_fails_non_git_cwd():
    code, _ = _run_statusline(
        '{"cwd": "/tmp"}',
        env_overrides={"BR8N_BACKEND": "local", "BR8N_DB_PATH": "/dev/null"},
    )
    assert code == 0
...
def test_never_fails_git_absent(tmp_path):
    code, _ = _run_statusline(
        ...
        env_overrides={"PATH": "/dev/null", "BR8N_BACKEND": "local"},
    )
    assert code == 0
...
def test_smoke_local_tier_no_capture(tmp_path):
    ...
    code, out = _run_statusline(
        ...
        env_overrides={"BR8N_BACKEND": "local", "BR8N_DB_PATH": db},
    )
    assert code == 0
