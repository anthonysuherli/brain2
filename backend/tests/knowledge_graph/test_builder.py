"""Unit tests for brain2.knowledge_graph.builder (stubbed extractor, no LLM calls)."""
...
async def test_build_graph_calls_extract_and_persists(tmp_path):
    ...
    with (
        patch("brain2.knowledge_graph.builder.extract_graph", new=AsyncMock(return_value=fake_extraction)),
        patch("brain2.knowledge_graph.builder.get_store", return_value=fake_store),
        patch("brain2.knowledge_graph.builder.embed_batch", new=AsyncMock(return_value=[[0.1] * 3])),
    ):
...
async def test_build_graph_empty_findings_returns_zeros():
    ...
    with (
        patch("brain2.knowledge_graph.builder.extract_graph", new=AsyncMock()) as mock_extract,
        patch("brain2.knowledge_graph.builder.get_store", return_value=fake_store),
    ):
        result = await builder.build_graph(fake_ctx, rebuild=True, use_schema=False)
...
async def test_build_graph_loads_schema_when_use_schema_true():
    ...
    with (
        patch("brain2.knowledge_graph.builder.extract_graph", new=AsyncMock(return_value=fake_extraction)) as mock_extract,
        patch("brain2.knowledge_graph.builder.get_store", return_value=fake_store),
        patch("brain2.knowledge_graph.builder.embed_batch", new=AsyncMock(return_value=[[0.1] * 3])),
    ):
...
async def test_build_graph_persists_edges():
    ...
    with (
        patch("brain2.knowledge_graph.builder.extract_graph", new=AsyncMock(return_value=fake_extraction)),
        patch("brain2.knowledge_graph.builder.get_store", return_value=fake_store),
        patch("brain2.knowledge_graph.builder.embed_batch", new=AsyncMock(return_value=[[0.1] * 3, [0.2] * 3])),
    ):
        result = await builder.build_graph(fake_ctx, rebuild=True, use_schema=False)
...
def test_load_intent_returns_none_on_malformed_schema(caplog):
    ...
    with caplog.at_level(logging.WARNING, logger="brain2.knowledge_graph.builder"):
        schema = builder._load_intent(store, "kb1")
    assert schema is None
