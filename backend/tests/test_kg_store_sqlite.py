...
from br8n.store import SQLiteStore, SupabaseStore
from br8n.store.sqlite import _ORG
...
async def test_upsert_nodes_returns_ids_in_order(store):
    ids = await store.upsert_kg_nodes(
        "akb",
        [_node("repo", "br8n"), _node("branch", "dev"), _node("file", "store/base.py")],
    )
    assert len(ids) == 3
...
async def test_upsert_reuses_existing_node_and_merges(store):
    [first] = await store.upsert_kg_nodes(
        "akb", [_node("repo", "br8n", properties={"path": "/a"}, grounded_in=["f1"])]
    )
    [second] = await store.upsert_kg_nodes(
        "akb", [_node("repo", "br8n", properties={"path": "/b", "extra": 1}, grounded_in=["f2"])]
    )
...
async def test_upsert_scopes_dedupe_to_kb(store):
    [a] = await store.upsert_kg_nodes("kb1", [_node("repo", "br8n")])
    [b] = await store.upsert_kg_nodes("kb2", [_node("repo", "br8n")])
    assert a != b  # same label in different KBs → different nodes


async def test_upsert_forces_local_org(store):
    [nid] = await store.upsert_kg_nodes("akb", [_node("repo", "br8n")])
...
async def test_upsert_edges_dedupes_and_drops_self_loops(store):
    a, b = await store.upsert_kg_nodes("akb", [_node("session", "s1"), _node("repo", "br8n")])
...
async def test_subgraph_seeded_pulls_neighbours(store):
    s, r, f = await store.upsert_kg_nodes(
        "akb", [_node("session", "s1"), _node("repo", "br8n"), _node("file", "x.py")]
    )
...
async def test_subgraph_full_when_no_seed(store):
    await store.upsert_kg_nodes("akb", [_node("repo", "br8n"), _node("branch", "dev")])
...
async def test_list_kg_nodes_type_filter_and_recency(store):
    await store.upsert_kg_nodes(
        "akb", [_node("session", "s1"), _node("repo", "br8n"), _node("session", "s2")]
    )
...
async def test_kg_stats_breakdowns(store):
    s, r = await store.upsert_kg_nodes("akb", [_node("session", "s1"), _node("repo", "br8n")])
    await store.upsert_kg_edges(
