...
The LLM task pass is disabled (``BR8N_ACTIVITY_LLM=0``) so these run offline:
...
from br8n.knowledge_graph.activity import _parse_diff_files, _repo_label, activity_extract
...
def _snap(**kw) -> WorkspaceSnapshot:
    base = dict(
        project_path="/Users/me/code/br8n",
        trigger="manual",
...
def test_repo_label_is_folder_basename():
    assert _repo_label(_snap(project_path="/Users/me/code/br8n")) == "br8n"
    assert _repo_label(_snap(project_path="br8n")) == "br8n"
...
async def test_structural_nodes_and_edges():
    snap = _snap(
        ...
        hypothesis="port KG builder to br8n",
    )
    ext = await activity_extract(snap, "finding-1")

    assert [n.label for n in _by_type(ext, "repo")] == ["br8n"]
    ...
    assert [n.label for n in _by_type(ext, "task")] == ["port KG builder to br8n"]
