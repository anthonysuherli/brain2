...
import br8n.knowledge_graph.activity as act
...
def local_env(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("BR8N_ACTIVITY_KG", "1")
...
async def test_rollup_is_newest_first_and_cross_repo(local_env):
    ...
    assert all(r["repo"] == "br8n" for r in rollup)
...
async def test_repo_filter_narrows(local_env):
    ...
    only = await act.query_activity(repo="br8n")
    assert all(
        (n.get("properties") or {}).get("repo") in (None, "br8n") or n.get("label") == "br8n"
        for n in only["nodes"]
    )
...
def test_schedule_is_noop_when_disabled(local_env, monkeypatch):
    monkeypatch.setenv("BR8N_ACTIVITY_KG", "0")
    fired = {}
