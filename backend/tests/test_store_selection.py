...
Selection order: BRAIN2_BACKEND env (explicit) → else cloud iff Supabase creds
...
from brain2.store import SQLiteStore, SupabaseStore, get_store
...
def test_explicit_local(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("BRAIN2_DB_PATH", str(tmp_path / "sel-local.db"))
...
def test_auto_local_when_no_creds(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("BRAIN2_DB_PATH", str(tmp_path / "sel-auto.db"))
    _reset_settings_cache()
    store_pkg._local_stores.clear()
    try:
        assert isinstance(get_store(), SQLiteStore)
...
def test_local_store_is_cached(monkeypatch, tmp_path):
    monkeypatch.setenv("BRAIN2_BACKEND", "local")
    monkeypatch.setenv("BRAIN2_DB_PATH", str(tmp_path / "sel-cache.db"))
    store_pkg._local_stores.clear()
    a = get_store()
