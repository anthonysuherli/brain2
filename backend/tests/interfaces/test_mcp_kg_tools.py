...
async def test_set_kg_schema_rejects_invalid():
    ...
    with patch(
        "br8n.interfaces.mcp.server.resolve_tenant", return_value=_mock_ctx()
    ):
        result = await server.br8n_set_kg_schema("proj", "kb", {"not": "a schema"})
...
async def test_set_kg_schema_rejects_empty_node_types():
    ...
    with patch(
        ...
        result = await server.br8n_set_kg_schema("proj", "kb", bad)
    assert result["ok"] is False
    assert "errors" in result
...
async def test_set_kg_schema_accepts_valid():
    ...
    with (
        patch("br8n.interfaces.mcp.server.resolve_tenant", return_value=_mock_ctx()),
        patch("br8n.interfaces.mcp.server.get_store", return_value=mock_store),
    ):
        result = await server.br8n_set_kg_schema("proj", "kb", _MINIMAL_VALID_SCHEMA)
...
async def test_get_kg_schema_returns_intent_and_emergent():
    ...
    with (
        ...
        result = await server.br8n_get_kg_schema("proj", "kb")
...
async def test_propose_kg_schema_returns_schema_dict():
    ...
    with (
        ...
        patch("br8n.interfaces.mcp.server.get_store", return_value=mock_store),
        patch(
            "br8n.interfaces.mcp.server.propose_schema",
            new=AsyncMock(return_value=mock_schema),
        ),
    ):
        result = await server.br8n_propose_kg_schema("proj", "kb")
...
async def test_kg_stats_returns_store_output():
    ...
    with (
        ...
        result = await server.br8n_kg_stats("proj", "kb")
...
async def test_graph_returns_nodes_edges_with_counts():
    ...
    with (
        ...
        result = await server.br8n_graph("proj", "kb")
...
async def test_graph_passes_focus_and_depth():
    ...
    with (
        ...
        result = await server.br8n_graph("proj", "kb", focus="br8n", depth=1)
...
async def test_build_graph_returns_counts():
    ...
    with (
        patch("br8n.interfaces.mcp.server.resolve_tenant", return_value=_mock_ctx()),
        patch(
            "br8n.interfaces.mcp.server.build_graph",
            new=AsyncMock(return_value=build_result),
        ),
    ):
        result = await server.br8n_build_graph("proj", "kb")
...
async def test_build_graph_forwards_kwargs():
    ...
    with (
        patch("br8n.interfaces.mcp.server.resolve_tenant", return_value=_mock_ctx()),
        patch("br8n.interfaces.mcp.server.build_graph", new=mock_build),
    ):
        await server.br8n_build_graph("proj", "kb", max_findings=10, rebuild=False, use_schema=False)
