...
the offline unit suite. Enable it with:
    ...
    BR8N_RUN_ISOLATION_IT=1
    BR8N_BACKEND=cloud
    SUPABASE_URL / SUPABASE_ANON_KEY / SUPABASE_SERVICE_ROLE_KEY / SUPABASE_JWT_SECRET
    BR8N_TEST_USER_A_EMAIL / BR8N_TEST_USER_A_PASSWORD
    BR8N_TEST_USER_B_EMAIL / BR8N_TEST_USER_B_PASSWORD
...
_ENABLED = os.getenv("BR8N_RUN_ISOLATION_IT") == "1"

pytestmark = pytest.mark.skipif(
    not _ENABLED,
    reason="cloud cross-user isolation IT — set BR8N_RUN_ISOLATION_IT=1 (+ cloud creds and two test users)",
)
...
def test_user_b_cannot_see_user_a_activity(client):
    token_a = _login(
        os.environ["BR8N_TEST_USER_A_EMAIL"], os.environ["BR8N_TEST_USER_A_PASSWORD"]
    )
    token_b = _login(
        os.environ["BR8N_TEST_USER_B_EMAIL"], os.environ["BR8N_TEST_USER_B_PASSWORD"]
    )
