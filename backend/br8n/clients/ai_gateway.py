...
async def stream_completion(
    *,
    ...
    """Stream a chat completion as text deltas (for the funnel chat agent).

    Yields content fragments as they arrive. Falls back to ``fallback_model`` if the
    primary model errors *before any token streams* (mid-stream failures surface as a
    raised exception the caller wraps). Plain text out — no JSON shape — so it's the
    streaming sibling of ``structured_completion``."""
    models = [model]
        ...
        try:
            ...
            async for chunk in stream:
                ...
                if delta:
                    yield delta
            return
        except Exception as exc:  # noqa: BLE001 — try the fallback model before giving up
            last_exc = exc
            logger.warning("stream_completion failed on model=%s: %s", m, exc)
