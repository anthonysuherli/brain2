...
from br8n.config import DriftConfig
    ...
    *,
    ...
    if intent is None:
        if total >= cfg.cold_start_min_nodes:
            ...
            line = (
                f"br8n has collected {total} graph nodes and no schema yet — "
                f"design one to organize them? `/br8n:schema`"
            )
            return DriftVerdict(
