...
**br8n adaptation:** the LLM returns label-based edges (``_LLMEdge.source`` /
...
canonical br8n ``KGExtraction`` shape.
"""
...
from br8n.config import KnowledgeGraphConfig
...
def _schema_block(schema: KGSchema) -> str:
    ...
    Adapts from delapan: br8n's ``KGSchema.relation_validity`` is a flat
    ...
    # br8n relation_validity is list[RelationValidity] — render as "src→tgt" pairs.
    validity_pairs = ", ".join(
...
def _to_kg_extraction(llm: _LLMExtraction) -> KGExtraction:
    """Convert LLM-facing label-based extraction to br8n's index-based ``KGExtraction``.
...
async def extract_graph(
    ...
    Returns a ``KGExtraction`` with integer-indexed edges (br8n canonical shape).
    """
    if not findings:
        return KGExtraction()
