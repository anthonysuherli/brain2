...
2. **Index-based edges** — br8n's ``KGExtraction.edges`` use integer indices
...
- ``schedule_kg_update`` / auto-trigger after explore — not implemented; add when
  the explore pipeline is wired in br8n.
...
from br8n.knowledge_graph.extractor import norm, extract_graph
from br8n.knowledge_graph.models import KGEdge, KGExtraction, KGNode, KGSchema
...
def _collapse_nodes(nodes: list[KGNode]) -> list[KGNode]:
    ...
    Note: br8n's ``KGNode`` has no ``aliases`` field — aliases are stored in
...
def _load_intent(store, kb_id: str) -> KGSchema | None:
    ...
    ``get_kg_intent`` returns ``{"version": N, "schema": <dict>}`` — br8n's
...
async def build_graph(
    ...
    **Incremental build** (``finding_ids`` parameter) — not implemented in v1.
    ...
    br8n and ``match_kg_nodes``-based vector dedupe is needed.
    ...
    if finding_ids:
        raise NotImplementedError(
            "Incremental KG build (finding_ids) is not implemented in br8n v1. "
