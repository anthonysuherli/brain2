"""The curated doc-tree distiller — note Findings → rendered ``.br8n/docs/`` tree.
...
- ``_infer_topics`` mirrors ``_task_label`` — same ``BR8N_LIVING_DOCS_LLM`` env gate,
...
- ``schedule_distill`` mirrors ``schedule_activity_update`` — module-level ``_BG_TASKS``
  set with ``add_done_callback(_BG_TASKS.discard)``, gated by ``BR8N_LIVING_DOCS``.
"""
...
from br8n.livingdocs.state import load_state, save_state, should_distill
...
async def _infer_topics(notes: list[dict]) -> list[str | None]:
    ...
    Mirrors ``activity._task_label``: ``BR8N_LIVING_DOCS_LLM=0`` (or any failure /
...
async def run_distill(ctx: TenantContext, *, project_path: str, kb: str) -> dict:
    """Distill the KB's note Findings into the curated ``.br8n/docs/`` tree.
...
def schedule_distill(ctx: TenantContext, *, project_path: str, kb: str) -> None:
    ...
    No-op when ``BR8N_LIVING_DOCS=0``. Mirrors ``activity.schedule_activity_update``:
