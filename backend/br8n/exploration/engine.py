...
from br8n.clients import tavily
from br8n.config import ExplorationConfig
from br8n.exploration.evaluator import evaluate_findings
from br8n.exploration.extractor import extract_findings
from br8n.exploration.merger import FindingMerger
from br8n.exploration.models import (
    ExplorationPlan,
    Finding,
    SearchQuery,
    Source,
)
from br8n.exploration.planner import plan_queries
...
async def run_exploration(
    ...
    *,
    ...
    try:
        ...
        plan = await plan_queries(prompt, cfg)

        # Phase 2 — search
        await progress("searching")
        search_results = await _run_search(plan.search_queries, cfg)
        ...
        # Phase 4 — extract, then reflect (evaluator-optimizer): a batched critic
        # scores each finding's signal quality and drops vacuous ones before merge.
        await progress("extracting")
        extraction_results = await _run_extraction(
            content_by_url, url_to_query, plan.extraction_prompt, cfg
        )
        ...
        # Phase 5 — merge (dedup + quality-blended confidence before persistence)
