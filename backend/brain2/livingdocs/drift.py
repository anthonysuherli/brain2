...
(``scripts/brain2-statusline.py``) is a zero-dependency script that keeps an in-sync
...
``brain2.knowledge_graph.drift`` (KG-schema drift).
...
# Same threshold the statusline uses — keep in sync with scripts/brain2-statusline.py.
DRIFT_FILES_WARN = 2
