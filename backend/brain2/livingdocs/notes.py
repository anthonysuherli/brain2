...
`.brain2/notes/<kb>/`. This mirrors snapshot persistence in
`brain2/capture/service.py` — same embed→insert→schedule_rebuild shape.
"""
...
async def persist_note(
    ...
    *,
    ...
    row = {
        ...
        "provenance": [
            {
                "source": f"brain2-livingdocs-{source}",
                "session": session_id,
                "path": project_path,
            }
        ],
