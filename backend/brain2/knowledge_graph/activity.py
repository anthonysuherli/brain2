...
power the ``brain2_activity`` tool, the cross-repo resume card, and ``/v1/activity``.
...
from brain2.config import ActivityConfig, get_config
from brain2.knowledge_graph.models import KGEdge, KGExtraction, KGNode
from brain2.store import Store, get_store
...
_TASK_SYSTEM = (
    ...
    "Rules: 3-7 words, imperative voice (e.g. 'port KG builder to brain2'), no "
...
async def _task_label(hypothesis: str, cfg: ActivityConfig) -> str:
    ...
    LLM pass is disabled (``BRAIN2_ACTIVITY_LLM=0``) or fails — the graph still grows."""
    raw = hypothesis.strip()
    if os.getenv("BRAIN2_ACTIVITY_LLM", "1") == "0":
