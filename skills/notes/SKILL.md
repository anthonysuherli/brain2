---
name: notes
description: Dictate what kind of session notes brain2 takes for this repo+branch — show the current note policy (section template + steer), edit it inline with free-text guidance, or launch a guided wizard to co-design it. Use when the user wants to control what end-of-session notes capture ("focus on architecture", "skip dep bumps"), review the policy, or set it up from scratch with `--wizard`.
---

# brain2 — Notes (steer the session-note policy)

brain2 writes a session note at the end of every conversation
...
| Invocation | Mode | Do |
|---|---|---|
| `/brain2:notes` (no args) | **Show** | Step A — print the current policy |
| `/brain2:notes <free text>` | **Steer** | Step B — set the steer to that text |
| `/brain2:notes --wizard` | **Wizard** | Step C — run the guided co-design |

## Step A — Show the current policy

Call
`mcp__plugin_brain2_brain2__brain2_notes_policy_get(project, kb, project_path)`.
...
The args are the new steer (e.g. `/brain2:notes focus on architecture, skip dep bumps`).

1. `brain2_notes_policy_get(project, kb, project_path)` — fetch the current policy.
2. Keep its `sections` exactly as-is; replace `steer` with the user's text.
3. `mcp__plugin_brain2_brain2__brain2_notes_policy_set(project, kb, project_path, policy=<updated>)`.
...
via `brain2_notes_policy_set` at a turn boundary. The wizard only runs on an explicit
