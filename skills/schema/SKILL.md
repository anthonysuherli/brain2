...
The wizard ends by calling `br8n_set_kg_schema` (versions automatically) and
offering `br8n_build_graph(use_schema=True, rebuild=True)`. The rebuild re-types
...
call `mcp__plugin_br8n_br8n__br8n_mark_drift_offered(project, kb, residual)`
...
stamp is what keeps br8n from nagging.
...
So br8n stays quiet until drift earns the interruption, then asks **once**.
