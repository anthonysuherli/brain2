...
**Opt-in only.** This wizard runs *only* when the user invokes `/brain2:notes --wizard`.
It is never auto-launched, never surfaced mid-flow. Like every brain2 gated decision:
...
Only then persist:
`mcp__plugin_brain2_brain2__brain2_notes_policy_set(project, kb, project_path, policy=<the approved {sections, steer}>)`.
