...
free-form extraction. Adapted from the KG-create wizard: brain2 grounds on a KB's
...
stop and suggest running `/brain2:explore` to fill coverage gaps first.

**Mode select:** call `mcp__brain2__brain2_get_kg_schema(project, kb)`. If `intent`
...
1. `mcp__brain2__brain2_propose_kg_schema(project, kb)` → a DRAFT ontology grounded
...
2. `mcp__brain2__brain2_kg_stats(project, kb)` → category distribution + confidence
...
4. Pick **3–5 "hot" findings** to use as grounding examples in Stage 3: highest
   ...
   `mcp__brain2__brain2_resume` to surface the preamble and hot findings, or work
...
1. `mcp__brain2__brain2_set_kg_schema(project, kb, schema)`. On
...
2. Offer to build: run `mcp__brain2__brain2_build_graph(use_schema=True, rebuild=True)`
   ...
   (never dropped).
3. After a build, `mcp__brain2__brain2_get_kg_schema(project, kb)` returns
...
4. On **update**, run a shortened Stage 3 (ground only new/changed types), then
   Stages 4–5. `mcp__brain2__brain2_set_kg_schema` versions automatically; the build
...
cues `/brain2:explore` to fill coverage gaps.
