---
name: project-init
description: First-run project initialization subagent — seeds a repo-scoped br8n KB from local context (repo/git/metadata) + bounded web enrichment, then signals completion for the schema offer. Dispatched by the SessionStart hook on repos with no existing br8n KB. Internal skill — not invoked by the user directly.
---
...
Collect and persist findings via `mcp__br8n__br8n_capture`. Work through each
category below in order. Creating the KB (the first `br8n_capture` call with
...
2. Run **one** `mcp__br8n__br8n_explore` call with a focused prompt scoped to
   those 2–4 facts, with `max_findings` set to 4–6.
3. If `br8n_explore` fails, times out, or returns an error: skip Phase B entirely
   and proceed to the output contract. Phase A findings are sufficient for a first
   draft.

Do not run more than one `br8n_explore` call. Do not loop on failure.
...
- Web budget is fixed: one `br8n_explore` call, `max_findings` ≤ 6
...
- If any individual step fails, log it internally and continue — partial findings
  are better than nothing and the KB can be enriched later via `/br8n:explore`
