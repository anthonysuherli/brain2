...
or fail the session. If the gate `BRAIN2_LIVING_DOCS=0` is set, the hook stays silent
...
1. **Fetch the note policy.** Call
   `mcp__brain2__brain2_notes_policy_get(project, kb, project_path)`. It returns the
...
## Persist with brain2_note

One call writes the note as both a searchable `note` Finding **and** a markdown file
under `.brain2/notes/<kb>/`, then schedules a debounced re-distill of the curated doc
tree:

```
mcp__brain2__brain2_note(
