# WS3 — Pricing & Packaging (brain2)

Status: draft for launch. Owner: solo founder.
Last updated: 2026-05-29.

This is the pricing & packaging workstream for brain2 — a context-capture-and-resume
...
cross-repo **SEARCH**, team **SHARING**, and **MANAGED KEYS**. brain2 is distributed
as a Claude Code plugin + a VS Code extension.

Grounding: this doc builds on the brain2 `dev` KB. Cited finding ids inline are
...
usage-based.** brain2's architecture already *is* the freemium boundary: the free
...
- **Not open-core.** Open-core gates the *source*: a free OSS core plus closed
  commercial modules. brain2's split is by *hosting/identity*, not by withholding
  ...
  but brain2 captures that same bottom-up adoption through a genuinely free,
...
brain2's aha moment is the first 30-second resume card. That stays free forever.

### FREE forever (local tier)

The entire single-machine loop, with no caps that touch the core job:

- Unlimited capture (`brain2_capture`) and resume (`brain2_resume`) on this machine.
- Full preamble + coverage banding, synopsis, and the resume-card webview.
- Local gap-fill explore (`brain2_explore`) **using the user's own API key** — the
  engine runs locally; the user pays their own LLM/search bill (BYO key).
- All current repos/branches stored locally in `~/.brain2/brain.db`.
...
| **MANAGED KEYS** | brain2-managed LLM/search keys so explore "just works" with no BYO-key setup; metered fairly inside the seat with a generous monthly allowance, hard overage cap (no surprise bills). | Team (Pro can opt in) |
...
brain2's closest structural analog is **Obsidian**: a free, local-first app whose
paid product is sync ($4–5/mo). That anchors the floor. But brain2 also delivers
active developer-workflow value (resume cards, explore), closer to Copilot/Tabnine
($10–12). Positioning brain2 between Obsidian Sync and Copilot Pro is correct: more
...
| **Team** | **$12/user/mo** | **$10/user/mo** | **$120/user/yr** | Sharing + managed keys + admin. Matches Raycast Teams ($12/user/mo) exactly — a known-good per-seat point for developer-productivity tools. Managed keys justify the premium over Pro (brain2 now carries some LLM/search COGS). |

Rationale notes:
- $8 Pro deliberately sits **below** the $10–20 AI-coding-assistant band: brain2 is a
  companion, not a replacement for Copilot/Cursor, and a developer will already be
  paying for one of those. Pricing under $10 keeps brain2 an easy *additive* purchase,
...
1. **New machine / second device detected** — user runs brain2 on a machine with an
...
2. **Cross-repo search miss** — user runs `/brain2:search` or resume and the answer is
...
6. **Explore BYO-key friction** — user's own key is missing/rate-limited/errors during
   `brain2_explore`. "Skip key setup — let brain2 manage it on Team." [Gated behind
...
2. **Where is my data on the free tier?** Only on your machine (`~/.brain2/brain.db`),
...
3. **What exactly does Pro add?** Sync across machines + search across all your repos,
   plus cloud backup. Your data moves to your brain2 cloud account.
...
KB findings (brain2 `dev`): `f21b3893`, `b04f6d13`, `1aae84be`, `d573d341`, `c7e9c440`.
