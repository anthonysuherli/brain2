# br8n — Efficient Growth Plan (highest-leverage sequence)
...
- [ ] **Both loops proven** — live `capture → resume` on local *and* cloud tiers;
  `python -m br8n.api.main --check` prints ✓db ✓sqlite-vec ✓loopback.
...
- [ ] Publish the marketplace repo publicly; verify `/plugin install br8n@br8n`
...
- [ ] Publish with a **keyword-rich display name**:
  *"br8n — Resume Card / Context Capture (Claude Code, git)."*
