...
**North Star:** weekly context-restoring resumes (a `/brain2:resume` that the developer
acts on after a real interruption — not a test invocation)

This workstream owns the path from "developer hears about brain2" to "developer has a
weekly resume habit." The thesis: brain2's time-to-value moment is **the first resume
...
Grounded in the brain2 `dev` KB (coverage: *sparse* on this query — see gaps at end).
...
here: brain2 must *show* value in the product, never *tell* it in a brochure.
...
For brain2 the critical path has exactly three nodes, and the goal is to traverse all
...
| Step | What happens (brain2-specific) | The value moment | Friction today |
|---|---|---|---|
| **0. Install** | `/plugin marketplace add` → `/plugin install brain2@brain2` (Claude Code) **or** install VS Code extension. Backend needs `brain2` + deps (`sqlite-vec`) in `backend/.venv`. | None yet — pure cost | Plugin reload required before MCP connects; Python venv + `sqlite-vec` build; which tier am I on? |
| **1. First capture** | Auto-fires on the **first interruption** the dev hits anyway — window **blur**, **git checkout** (`.git/HEAD` change), or **idle ≥300s** (`triggers.ts`). Snapshots branch, open/cursor files, `git_diff_stat`, one-line hypothesis. Debounced 30s. Manual `/brain2:capture` also works. | "It noticed I left without me asking." | Dev must produce *one* interruption in-session; needs a hypothesis line; silent if backend isn't running |
| **2. First resume** | On focus-regain after a blur capture, the extension sets `pendingResume` and offers the card (`extension.ts`); or dev runs `/brain2:resume`. Card leads with the **latest hypothesis** ("You were: …") + files + diff stat, on `ViewColumn.Beside`. Coverage routes: `rich`/`sparse` → show card, `gap` → offer explore. | **THE AHA** — "30 seconds and I'm back where I was, not 9.5 minutes." | Empty first card if no capture happened yet; `gap` coverage on a brand-new KB can underwhelm |
...
- [ ] **Plugin reload gap.** After `/plugin install brain2@brain2` the MCP server only
...
- [ ] **MCP connect verification.** Dev can't tell if `brain2_*` tools are live. *Fix:*
  ship a `/brain2:resume` that, on connection failure, returns a one-line diagnostic
  ("backend not reachable at `BRAIN2_API_URL` — is `python -m brain2.api.main` running?")
  instead of a silent error. Make the **status bar item** (`$(brain) brain2`, already in
...
- [ ] **Backend Python deps.** `sqlite-vec` + editable install trip people up. *Fix:*
  ...
  `python -m brain2.api.main --check` health command that prints "✓ db, ✓ sqlite-vec,
...
- [ ] **Tier ambiguity.** Dev doesn't know if they're local or cloud. *Fix:* echo the
  resolved backend on first boot ("brain2: local tier (SQLite at `~/.brain2/brain.db`,
...
> The developer is interrupted, returns, opens `/brain2:resume`, and the card replays
...
maps cleanly: **trigger** = the interruption brain2 already detects; **routine** = open
the resume card; **reward** = instant re-entry. brain2 owns the trigger *and* the reward,
...
so a secondary health metric is **repos with ≥3 captures** (the dev has wired brain2 into
...
**Pick:** shareable / showcase loop on the resume card. brain2 already *produces a
...
2. **"Share this resume"** exports the card as a self-contained HTML / image / gist —
   ...
   gate). A small "↻ resumed in 30s with brain2" footer + install link is the payload.
3. Dev drops it in a PR description, a standup thread, or a tweet ("this is how I picked up
   a 2-day-old branch") — the peer-led channels brain2's ICP actually trusts [#f21b3893].
...
brain2's re-engagement is a **contextual trigger**, the strongest kind — surface the
...
- **Lapsed-repo nudge (cloud/email, future):** Day-1/3/7 micro-retention checkpoints
  ...
  brain2's real trigger is the interruption itself.
...
> brain2 saves your context automatically the moment you step away — switch windows,
> check out a branch, or go idle for 5 minutes, then come back here.
> *Want to test it now?* Run **/brain2:capture** to snapshot where you are, then reopen
...
> **brain2 can't reach its engine.**
> The local engine isn't responding at `<BRAIN2_API_URL>`. Start it with
> `python -m brain2.api.main` (free local tier — no API key needed, loopback only), or run
> `python -m brain2.api.main --check` to see what's missing. Then reopen this card.
...
patterns, or context-tool retention curves. Recommend a `/brain2:explore` pass on
...
- KB findings: `[#f21b3893]` developer buying behavior, `[#43300575]` month-one SaaS benchmarks (brain2 `dev` KB)
