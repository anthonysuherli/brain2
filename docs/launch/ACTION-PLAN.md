...
- [ ] Verify the **free local loop** too; ship `python -m br8n.api.main --check` → prints ✓db ✓sqlite-vec ✓loopback. *(WS-8)*
...
- [ ] Build the **shareable resume card** export (redacted to intent + filenames, "↻ resumed in 30s with br8n" footer) — the growth loop. *(WS-8 §4)*
...
- [ ] Go **live on marketplaces**: Claude Code plugin (`/plugin install br8n@br8n`) + VS Code Marketplace with the keyword-rich name *"br8n — Resume Card / Context Capture (Claude Code, git)."* *(WS-2 ch.2/3)*
...
- [ ] Ship the **"two weeks of br8n: installs, what broke, what's next"** build-in-public post.
...
**WS-5 (legal/privacy/trust)** — br8n stores code. Treat the LLC, the lawyer-reviewed
