# brain2 — The Most Efficient Path to Users (Strategy Brief)
...
efficient way to gain users on brain2?*
...
The most efficient lever for brain2 is **not** a generic dev-tool playbook — it is
that brain2 already ships into the single fastest-moving distribution channel in
...
plugins in it are low quality. brain2 pairs:
...
- PLG winners optimize **Time-To-First-Value**; the freemium benchmark is **< 15 min**.
  Lovable hit $100M ARR in 8 months on this discipline. brain2's local loop can
  plausibly hit **< 2 min** (install → `/brain2:capture` → `/brain2:resume`).
...
|---|------|:---:|---|
| 1 | **Own the Claude Code plugin / MCP discovery surface** | **S** | One-command `/plugin install` + `claude mcp add`; community directories (claudemarketplaces.com) + `awesome-*` lists are near-zero-cost listings that put you in front of devs already sold on AI coding. brain2 already has the manifest + marketplace.json + `.mcp.json`. **Highest leverage, lowest effort.** |
...
Efficiency optimizes the *funnel*; it assumes the *trust precondition* is met. brain2
