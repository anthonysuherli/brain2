...
brain2 captures and stores **sensitive developer context**: git branch, open/cursor
file paths, `git diff --stat`, and a one-line hypothesis of intent. On the **free
local tier** this never leaves the machine (SQLite at `~/.brain2/brain.db`,
...
Because brain2 stores **sensitive code/git data**, the liability exposure is higher
...
- **Sales tax:** SaaS is taxable in some US states and not others. This is the single
  ...
  liability entirely. brain2 KB finding `b04f6d13` already costs this: ~$230/mo premium
...
— brain2's data class (source-code-adjacent) is sensitive enough to warrant one paid
...
- **Customer data / IP ownership** — *the customer owns their code and captured context;
  brain2 claims no ownership over customer content.* This is critical for a code tool;
  state it plainly.
- **License to process** — limited license to process captured content **only to provide
  ...
  **brain2 does not train models on customer code.**
...
- **Roles:** customer = controller, brain2 = processor; subject matter = code-context
  snapshots.
...
> Snapshots are stored in a local SQLite database at `~/.brain2/brain.db`. The API binds
> to `127.0.0.1` (loopback only) and requires no account and no API key. Nothing is
> transmitted to brain2 servers, Supabase, or any LLM/embedding API. If you never enable
...
This is brain2's strongest and most honest trust line, and it's a real differentiator:
...
local-first peers like Dyad). brain2's free tier *can*, because the architecture enforces
...
> Your snapshots live in a SQLite file on your disk. brain2 binds to localhost only and
