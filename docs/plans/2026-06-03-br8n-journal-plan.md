# br8n Journal Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Add a cross-project, write-anytime, semantically-searchable personal journal to br8n, reusing its existing OpenAI embeddings + vector search.

**Architecture:** A journal entry is a br8n finding (`category="journal"`) under a reserved, repo-independent scope (`__journal__`), stored in the same `~/.br8n/brain.db`. New thin write/search paths sit beside the existing note path; the one infra change extends `match_findings` to support org-wide (`kb_id=None`) + category-filtered search so `scope='both'` can span the journal and every project's notes.
...
**Spec:** `docs/plans/2026-06-03-br8n-journal-design.md` (commit `dd3a7b7`).

**Conventions (read before starting):**
- All commands run from `/Users/suherli/Repositories/br8n/backend` unless noted.
- Tests wire the real `SQLiteStore` via `BR8N_BACKEND=local` + a tmp `BR8N_DB_PATH`; only the embedder is faked (no OpenAI call). Clear `br8n.store._local_stores` before and after so the tmp DB is honored. Mirror `backend/tests/test_note_tool.py`.
...
**Files:**
- Modify: `backend/br8n/store/base.py:25-33` (protocol signature + docstring)
- Modify: `backend/br8n/store/sqlite.py:170-212` (`match_findings` impl)
- Modify: `backend/br8n/store/supabase.py:72-92` (`match_findings` impl)
...
In `backend/br8n/store/base.py`, replace the `match_findings` protocol method (lines 25-33) with:
...
In `backend/br8n/store/sqlite.py`, replace `match_findings` (lines 170-212) with:
...
In `backend/br8n/store/supabase.py`, replace `match_findings` (lines 72-92) with:
...
```bash
git add backend/br8n/store/base.py backend/br8n/store/sqlite.py backend/br8n/store/supabase.py supabase/migrations/0008_match_findings_org.sql backend/tests/test_match_findings_global.py
...
The journal's storage primitive: a reserved scope constant and a near-clone of `persist_note` that writes a `category="journal"` finding plus a markdown mirror in the **global** `~/.br8n/journal/` dir — and never schedules a synopsis rebuild.

**Files:**
- Create: `backend/br8n/constants.py`
- Create: `backend/br8n/livingdocs/journal.py`
...
Expected: FAIL — `No module named 'br8n.constants'` / `br8n.livingdocs.journal`.
...
```bash
git add backend/br8n/constants.py backend/br8n/livingdocs/journal.py backend/tests/test_journal_tool.py
git commit -m "feat(journal): persist_journal + reserved __journal__ scope"
```

---

### Task 3: `br8n_journal` write MCP tool
...
**Files:**
- Modify: `backend/br8n/interfaces/mcp/server.py` (add import + `_journal_impl` + `br8n_journal` after `br8n_distill`, around line 170)
...
```python
async def test_br8n_journal_tool_writes_entry(tmp_path, monkeypatch):
    monkeypatch.setenv("BR8N_BACKEND", "local")
    monkeypatch.setenv("BR8N_DB_PATH", str(tmp_path / "brain.db"))
...
Run: `pytest tests/test_journal_tool.py::test_br8n_journal_tool_writes_entry -v`
Expected: FAIL — `server` has no attribute `_journal_impl`.

- [ ] **Step 3: Implement the tool**

In `backend/br8n/interfaces/mcp/server.py`, add to the imports near line 36-38:
...
Then insert, immediately after the `br8n_distill` tool (after line 169):
...
```bash
git add backend/br8n/interfaces/mcp/server.py backend/tests/test_journal_tool.py
git commit -m "feat(journal): br8n_journal MCP write tool"
```

---

### Task 4: `br8n_journal_search` + `br8n_journal_recent` MCP tools
...
**Files:**
- Modify: `backend/br8n/interfaces/mcp/server.py` (add `_journal_search_impl` + two tools after `br8n_journal`)
...
In `backend/br8n/interfaces/mcp/server.py`, insert after the `br8n_journal` tool (from Task 3):
...
```bash
git add backend/br8n/interfaces/mcp/server.py backend/tests/test_journal_search.py
git commit -m "feat(journal): br8n_journal_search + br8n_journal_recent tools"
```
...
The journal write creates a `__journal__` project/KB. It must not appear in `br8n_projects` (which powers the `/br8n:pickup` selector). Filter the sentinel in both `list_projects` impls. (Note: it never enters the activity graph — that's fed only by `br8n_capture`, which the journal does not call.)

**Files:**
- Modify: `backend/br8n/store/sqlite.py:453-478` (`list_projects`)
- Modify: `backend/br8n/store/supabase.py:252-...` (`list_projects` project query)
...
In `backend/br8n/store/sqlite.py`, add the import near the top (after line 42, the `_ORG` definition):
...
In `backend/br8n/store/supabase.py`, add the import near the other top-level imports:
...
```bash
git add backend/br8n/store/sqlite.py backend/br8n/store/supabase.py backend/tests/test_journal_search.py
git commit -m "feat(journal): hide __journal__ scope from project listings"
```

---

### Task 6: `/br8n:journal` skill + plugin registration
...
Run (from repo root `/Users/suherli/Repositories/br8n`):
...
git commit -m "feat(journal): /br8n:journal skill + plugin registration"
```
...
Reload the br8n plugin in Claude Code, then in any repo:
`/br8n:journal noticed the journal reuses the findings table cleanly`
`/br8n:journal search journal --scope journal`
Confirm the entry is written (a file appears under `~/.br8n/journal/`) and search returns it.
...
- `br8n_journal` write tool (any-time) → Task 3. ✓
- `br8n_journal_search` (journal/project/both) + `br8n_journal_recent` → Task 4. ✓
...
- `/br8n:journal` skill + registration → Task 6. ✓
...
**Placeholder scan:** No TBD/TODO; every code + test step is complete. `br8n_journal_recent` dropped the inert `days` param (no date filter exists) to avoid a configured-but-inert knob.

**Type consistency:** `match_findings(kb_id: str | None, …, categories=None)` is identical across base/sqlite/supabase and all call sites. `persist_journal` returns `{finding_id, entry_path}`; `_journal_impl` adds `scope`; tests assert exactly those keys. `JOURNAL_SCOPE` is defined once in `br8n/constants.py` and imported everywhere (no duplicated literal, no import cycle).
