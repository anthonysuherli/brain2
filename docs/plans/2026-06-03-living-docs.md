...
**Goal:** Make br8n auto-generate and continuously update a two-layer, on-disk documentation tree (append-only session notes → debounced curated docs), plus a background change-driven auto-capture watcher — all non-blocking and fail-silent.

**Architecture:** A new `br8n/livingdocs/` backend package owns paths (`.br8n/` in the repo, git-ignored), the note policy (template + free-text steer), the docs-state bookkeeping, note persistence (note = a `category="note"` Finding *and* a markdown file), the snapshot→note fallback distiller, and the curated-tree distiller (content-inferred taxonomy, schema-optional). New MCP tools (`br8n_note`, `br8n_notes_policy_{get,set}`, `br8n_distill`) are the agent-facing surface; new skills (`/br8n:notes` with a HITL wizard, `/br8n:docs`) drive them. A `SessionStart` hook launches a non-LLM Python **watcher subprocess** that polls git and captures on real change; a `SessionEnd`/`Stop` hook writes the session note then signals a debounced distill. Everything degrades to silence when its env gate is off or anything fails.
...
**Conventions:** Imports always `from br8n.*`. Run tests with `BR8N_BACKEND=local`. New env gates default **on** and are read with `os.getenv("X", "1") == "0"` guards (mirror `schedule_activity_update`). Every background path wraps work in `try/except` that logs and never raises (mirror `_run_activity_update`). Best-effort writes into the user repo use the `project_path` carried by the snapshot/tool call.
...
**Worktree:** `.worktrees/living-docs` on branch `feat/living-docs`. Deps installed; baseline `BR8N_BACKEND=local .venv/bin/pytest -q` = 204 passed, 1 skipped. Run all commands from `backend/` with `.venv/bin/...`.
...
**Files:**
- Modify: `backend/br8n/config.py` (add class near `ActivityConfig` ~line 142; wire into `AppConfig` ~line 262)
...
**Step 2: Run it, expect FAIL** — `Run: BR8N_BACKEND=local .venv/bin/pytest tests/test_livingdocs_config.py -q` → `AttributeError: 'AppConfig' object has no attribute 'living_docs'`.
...
**Files:**
- Create: `backend/br8n/livingdocs/__init__.py` (empty docstring module)

**Step 1:** Write `backend/br8n/livingdocs/__init__.py`:
...
```bash
git add backend/br8n/livingdocs/__init__.py
...
**Files:**
- Create: `backend/br8n/livingdocs/paths.py`
...
git commit -m "feat(livingdocs): .br8n/ path resolver + layout bootstrap"
```
...
**Files:**
- Create: `backend/br8n/livingdocs/policy.py`
...
```bash
git add backend/br8n/livingdocs/policy.py backend/tests/test_livingdocs_policy.py
...
**Files:**
- Create: `backend/br8n/livingdocs/notes.py`
...
async def test_persist_note_writes_finding_and_file(tmp_path, monkeypatch):
    ...
    # File written under .br8n/notes/main/
    p = DocPaths(project_path=str(tmp_path), kb="main")
...
async def persist_note(
    ...
    *,
    ...
    row = {
        ...
        "tags": ["note", source],
        "provenance": [{"source": f"br8n-livingdocs-{source}", "session": session_id, "path": project_path}],
    }
...
```bash
git add backend/br8n/livingdocs/notes.py backend/tests/test_livingdocs_notes.py
git commit -m "feat(livingdocs): persist note as Finding + markdown file"
```

### Task 2.3: `br8n_note` MCP tool + `Stop` hook + `_shared/session-note.md`

**Files:**
- Modify: `backend/br8n/interfaces/mcp/server.py` (add tool near `br8n_capture`)
...
@pytest.mark.asyncio
async def test_br8n_note_tool(tmp_path, monkeypatch):
    monkeypatch.setenv("BR8N_BACKEND", "local")
    monkeypatch.setenv("BR8N_DB_PATH", str(tmp_path / "brain.db"))
    res = await server.br8n_note.fn(
...
```python
from br8n.livingdocs.distill import schedule_distill  # add at top
from br8n.livingdocs.notes import persist_note
...
async def br8n_note(
    ...
    rendered per the KB's note policy (see br8n_notes_policy_get)."""
...
1. read the note policy (`mcp__br8n__br8n_notes_policy_get`),
2. summarize the just-finished conversation into those sections + honoring the steer,
3. call `mcp__br8n__br8n_note(project, kb, project_path, content, session_id, title)`,
4. do it without blocking; if nothing substantive happened, skip.

Gate the whole hook on `BR8N_LIVING_DOCS != "0"`.
...
**Step 5:** Write `skills/_shared/session-note.md` — the canonical note-writing convention (the same instructions, referenced by the hook directive and reusable by `/br8n:notes` manual runs).

**Step 6: Run** `BR8N_BACKEND=local .venv/bin/pytest tests/test_note_tool.py -q` → PASS. **Commit**

```bash
git add backend/br8n/interfaces/mcp/server.py backend/tests/test_note_tool.py hooks/session-note.py hooks/hooks.json skills/_shared/session-note.md
git commit -m "feat(livingdocs): br8n_note tool + Stop hook + session-note convention"
```
...
## Step 3 — `/br8n:notes` skill + policy tools + wizard
...
**Step 2: Implement** `_policy_get_impl` / `_policy_set_impl` + thin `@mcp.tool()` wrappers `br8n_notes_policy_get` / `br8n_notes_policy_set` using `load_policy`/`save_policy`/`NotePolicy` and `DocPaths`. Return `{"policy": <dump>, "project", "kb"}`. `set` validates via `NotePolicy.model_validate(...)`; on validation error return `{"errors": [...]}`.

**Step 3: Run, PASS. Commit**

```bash
git add backend/br8n/interfaces/mcp/server.py backend/tests/test_policy_tools.py
git commit -m "feat(livingdocs): notes-policy get/set MCP tools"
```

### Task 3.2: `/br8n:notes` skill + wizard convention
...
- no args → `br8n_notes_policy_get` and print the current template + steer.
- `<free text>` → set the steer via `br8n_notes_policy_set` (keep existing sections).
- `--wizard` → run `_shared/notes-policy-wizard.md`.

**Step 2:** Write `skills/_shared/notes-policy-wizard.md` modeled on `kg-schema-wizard.md`: a HITL loop asking **one multiple-choice question at a time** (which sections to keep/add, level of detail, what to skip, free-text steer), then persist via `br8n_notes_policy_set` at a turn boundary. Reference the design's "offer once, opt-in" principle: never auto-launch; only on `/br8n:notes --wizard`.
...
git commit -m "feat(livingdocs): /br8n:notes skill + policy wizard"
```
...
**Files:**
- Create: `backend/br8n/livingdocs/watch.py`
...
from br8n.livingdocs.watch import fingerprint, changed
...
- `def run_watch(cwd, interval, stop_path)` — the loop: every `interval`s read git state, capture on change, exit when `stop_path` exists or parent died. Gate on `BR8N_AUTO_CAPTURE != "0"` and `BR8N_LIVING_DOCS != "0"`.
- `if __name__ == "__main__": run_watch(...)` reading args/env (`BR8N_WATCH_CWD`, `BR8N_WATCH_STOP`, interval from config).
...
```bash
git add backend/br8n/livingdocs/watch.py backend/tests/test_livingdocs_watch.py
...
- `should_launch(cwd) -> bool` — git repo? + gates on; + not already running (check a pidfile under the repo's `.br8n/`).
- `main()` — read stdin JSON, derive cwd, if `should_launch`: write a stop-file path, `subprocess.Popen([sys.executable, "-m", "br8n.livingdocs.watch"], env={..."BR8N_WATCH_CWD", "BR8N_WATCH_STOP"...}, start_new_session=True)` (detached), record pid. Silent on any failure.
...
**Files:**
- Create: `backend/br8n/livingdocs/fallback.py`
...
- `async def distill_fallback_note(ctx, *, project_path, kb, session_id) -> dict|None` — fetch the session's snapshots (`store.list_findings(kb_id, category="snapshot")`, filter by `provenance.session`/recency), call `synth_note_markdown`, then `persist_note(..., source="backend")`. Best-effort; returns None on no snapshots. Optionally LLM-upgrade the body when `BR8N_LIVING_DOCS_LLM != "0"` (reuse the activity task-LLM client pattern), falling back to the deterministic body on any failure.

**Step 3: Run, PASS. Commit**

```bash
git add backend/br8n/livingdocs/fallback.py backend/tests/test_livingdocs_fallback.py
...
**Files:**
- Create: `backend/br8n/livingdocs/state.py`
...
from br8n.livingdocs.paths import DocPaths
from br8n.livingdocs.state import load_state, save_state, DocsState, should_distill
...
```bash
git add backend/br8n/livingdocs/state.py backend/tests/test_livingdocs_state.py
...
**Files:**
- Create: `backend/br8n/livingdocs/distill.py`
...
- `async def _infer_topics(notes) -> list[str|None]` — gated LLM pass (`BR8N_LIVING_DOCS_LLM`) that tags each note with a short topic; deterministic fallback = `None` (→ flat). Mirror activity `_task_label` gating.
- `async def run_distill(ctx, *, project_path, kb) -> dict` — load note-Findings (`list_findings(kb_id, category="note")`), `_infer_topics`, `plan_layout` (schema from `get_kg_intent` node_types if set, else None), write/refresh `.br8n/docs/<folder>/<slug>.md` files (curated summary per cluster — concatenated/deduped note bodies; **not** re-ingested as Findings), update + save `DocsState` (reset `notes_since_distill`, stamp `last_distill_at`, store taxonomy). Best-effort.
- `schedule_distill(ctx, *, project_path, kb)` — fire-and-forget, debounced: bump `notes_since_distill`, `should_distill?` → `asyncio.create_task(run_distill(...))` held in a module `_BG_TASKS` set (mirror `schedule_activity_update`). No-op when `BR8N_LIVING_DOCS == "0"`.

**Step 3: Run, PASS. Commit**

```bash
git add backend/br8n/livingdocs/distill.py backend/tests/test_livingdocs_distill.py
...
**Step 1: Write the test** — persist `cluster_min_notes` notes via `_note_impl` (forcing distill by setting `debounce_n=1` via monkeypatched config or calling `run_distill` directly), assert files appear under `.br8n/docs/`. Keep it deterministic by calling `run_distill` directly (avoid racing the fire-and-forget task):
...
async def test_notes_distill_to_doc_tree(tmp_path, monkeypatch):
    monkeypatch.setenv("BR8N_BACKEND", "local")
    monkeypatch.setenv("BR8N_DB_PATH", str(tmp_path / "brain.db"))
    monkeypatch.setenv("BR8N_LIVING_DOCS_LLM", "0")  # deterministic → flat
    ctx = resolve_tenant("proj", "main", create=True)
    for i in range(3):
...
## Step 7 — `/br8n:docs` skill + `br8n_distill` tool

### Task 7.1: `br8n_distill` MCP tool
...
**Step 2: Implement** `_distill_impl` + `@mcp.tool() br8n_distill(project, kb, project_path, force=False)` — `force=True` calls `run_distill` immediately; else respects debounce via `schedule_distill` and reports whether it ran. Return doc-file count under `.br8n/docs/`.

**Step 3: Run, PASS. Commit**

```bash
git add backend/br8n/interfaces/mcp/server.py backend/tests/test_distill_tool.py
git commit -m "feat(livingdocs): br8n_distill tool (force/debounced)"
```

### Task 7.2: `/br8n:docs` skill

**Files:**
- Create: `skills/docs/SKILL.md`
- Modify: `.claude-plugin/plugin.json` (add `"./skills/docs"`)

**Step 1:** Write `skills/docs/SKILL.md` (frontmatter `name: docs` + description). Behavior: resolve project/kb/project_path; list/read `.br8n/docs/` (use the agent's own file tools to read the tree and present it); `--rebuild` → `br8n_distill(force=True)` then show the refreshed tree.
...
git commit -m "feat(livingdocs): /br8n:docs skill (browse + --rebuild)"
```
...
**Step 1:** `Run: BR8N_BACKEND=local .venv/bin/pytest -q` → expect **all prior 204 + new tests pass**, 0 failures. Fix any regressions (likely import-time issues if `schedule_distill` wiring is off).
...
**Step 1:** Document: new `br8n/livingdocs/` package; new env gates (`BR8N_LIVING_DOCS`, `BR8N_AUTO_CAPTURE`, `BR8N_AUTO_CAPTURE`-interval, `BR8N_LIVING_DOCS_LLM`, debounce N/T); new MCP tools (`br8n_note`, `br8n_notes_policy_{get,set}`, `br8n_distill`); new skills (`/br8n:notes`, `/br8n:docs`); new hooks (`Stop` session-note, `SessionStart` auto-capture, `SessionEnd` stop). Note `category="note"` Findings and the `.br8n/` git-ignored on-disk layout.
...
4. **2.1** policy → **2.2** notes → **6.2** distill (`schedule_distill`) → **2.3** `br8n_note` tool + Stop hook
5. **3.1, 3.2** policy tools + `/br8n:notes` + wizard
6. **4.1, 4.2** watcher + SessionStart/End wiring
7. **5.1** fallback distiller
8. **6.3** distill integration test
9. **7.1, 7.2** `br8n_distill` + `/br8n:docs`
