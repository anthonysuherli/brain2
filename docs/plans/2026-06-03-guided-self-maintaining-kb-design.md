# br8n — Guided, Self-Maintaining, Portable Personal KB — design
...
surfacing hook (D4) + `/br8n:schema` manifest registration added on `dev`. See §10
...
**One-liner:** *br8n — a self-maintaining, portable personal knowledge base you
manage agentically, guided at the one seam that matters: its schema.*

br8n watches you work, distills what it learns into one personal graph, and keeps it
...
| **Portable** | br8n-the-cloud is the provider: your per-user graph + concepts sync across workspaces & machines, so context follows you from `repoA`/laptop to `repoB`/desktop. | north-star (cloud sync listed *future* in CLAUDE.md) |
...
the schema. Everything else is inference br8n does alone. "Guided" is not a constant
...
- cold-start: *"br8n has collected enough to propose a knowledge schema — design it?
  `/br8n:schema`"*
- drift: *"Your activity no longer fits its schema (12 unplaced entities around
  'deployment', 'incidents') — reshape it? `/br8n:schema`"*
...
- **declined / ignored** → stamp and **stop re-raising every launch.** The capability
  stays available on demand (`/br8n:schema` always works).
...
Accepting the offer (or running `/br8n:schema` cold) launches a **HITL co-design** —
the one place br8n trades silence for dialogue. This **reintroduces delapan's
`propose_schema` + `kg-schema-wizard`** (currently excluded from the br8n fork per
...
|---|---|
| Grow it | auto-capture subagent + `/br8n:capture`, `/br8n:explore` |
| Query it | `/br8n:search`, `/br8n:activity`, `/br8n:resume`, `/br8n:context` |
| **Reshape it** | **`/br8n:schema` — the drift-triggered guided loop (this design)** |
| Curate it | `/br8n:notes` wizard, `/br8n:docs` |
...
| Surface | Change |
|---|---|
| `/br8n:schema` skill (`skills/schema/SKILL.md`) | **New** — runs the guided loop; `--rebuild` forces a pass on demand |
| `_shared/kg-schema-wizard.md` | **Reintroduced** from delapan — the one-question-at-a-time convention |
| `br8n_schema` (MCP) + `POST /v1/schema/propose` | **New** — propose a schema delta from a seed |
...
- `BR8N_SCHEMA_DRIFT` — master switch for the detector + offer.
- `BR8N_DRIFT_TAU_FIT` / `BR8N_DRIFT_TAU_SIM` — residual thresholds.
- `BR8N_DRIFT_WINDOW` (`W`), `BR8N_DRIFT_RATIO` (`ρ`), `BR8N_DRIFT_FLOOR`.
- `BR8N_SCHEMA_COLDSTART_N` (`N_min`), `BR8N_SCHEMA_REARM_DELTA` (`Δ_rearm`).

Mirrors the `BR8N_ACTIVITY_KG` / `BR8N_ACTIVITY_LLM` gating discipline. Best-effort:
...
4. `/br8n:schema` skill + `br8n_schema` + `_shared/kg-schema-wizard.md` — the
...
- **Reintroducing `propose_schema` risks a cross-repo dep.** Mitigation: port the file
  and rename imports `delapan.* → br8n.*`, exactly as the rest of the fork (CLAUDE.md
...
- Whether the cold-start offer should also seed from `/br8n:explore` web findings,
...
(`knowledge_graph/drift.py`, `br8n_schema_drift` MCP tool, `skills/schema/SKILL.md`,
...
| **D4 surfacing** | `SessionStart` hook auto-emits the offer at a turn boundary (§3) | **now built** (`hooks/first-run-init.py`): the existing-KB branch calls `pending_schema_offer` (mirrors `br8n_schema_drift` in-process) and injects `offer_line` only when `should_offer` — silent otherwise. Stamps via `mark_drift_offered`/`mark_init_offered`. `/br8n:schema` registered in `plugin.json`. | ✅ **Closed.** Computed in-process (not via a directive that forces a tool call every session), so a no-drift session stays fully silent — strictly more non-blocking. |
...
| **D6 re-type** | bounded re-type of just the residual seed; full rebuild deferred (§5) | **full graph rebuild** under the new schema (`br8n_build_graph(use_schema=True, rebuild=True)`); "full-rebuild only — no incremental today" | **Adopt as-built**, with a caveat: full re-extraction is more thorough but costlier on large graphs. The bounded-re-type / incremental path becomes the future optimization (§9). |
...
hook surfaces the gated offer at a turn boundary, and `/br8n:schema` runs the guided
