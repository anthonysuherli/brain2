# br8n First-Run Init + KG Schema Wizard — Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** When a user opens a repo br8n has never seen, a SessionStart hook detects the first run, Claude dispatches a background subagent that seeds a repo-scoped KB (local crawl + bounded web), then offers Delapan's full 5-stage KG schema wizard.

**Architecture:** Three layers, built bottom-up. (1) **Backend port** — bring Delapan's KG schema/builder Python layer into br8n, adapted to br8n's `Store` abstraction. (2) **MCP surface** — expose `propose_kg_schema`, `set_kg_schema`, `get_kg_schema`, `build_graph`, `graph`, `kg_stats`, and a cheap `kb_exists`. (3) **Plugin surface** — a SessionStart hook, two shared skill docs (`project-init.md`, `kg-schema-wizard.md`), and the `plugin.json` wiring + an `init_offered_at` offer-once stamp.

**Tech Stack:** Python 3.11, FastMCP (`mcp.server.fastmcp`), Supabase (Postgres+pgvector) via br8n's `Store` abstraction (local/sqlite/supabase backends), pytest (asyncio_mode=auto), ruff, pyright. Plugin layer: Claude Code hooks + markdown skills.

**Design doc:** `docs/plans/2026-06-01-br8n-first-run-init-design.md` (on `dev`).

**Source of truth for the port:** `/Users/suherli/Repositories/delapan/backend/delapan/knowledge_graph/{schema,builder,extractor,service,models}.py` and `delapan/interfaces/mcp/server.py:378-520`. **Read those before each port task.** br8n = `/Users/suherli/Repositories/br8n/backend`. Work in the worktree at `.worktrees/feat/first-run-init`.
...
```bash
cd /Users/suherli/Repositories/br8n/.worktrees/feat/first-run-init/backend
python -m venv .venv && source .venv/bin/activate
pip install -e ".[dev]"
cp /Users/suherli/Repositories/br8n/backend/.env .env 2>/dev/null || echo "no .env to copy — check secrets"
```
...
The foundation. Everything else is blocked on this. Port from Delapan, adapt to br8n's `Store`. **Keep `exploration/`-style purity where Delapan has it** — `schema.py` and `extractor.py` are pure (no Supabase); `service.py` and `builder.py` own persistence.
...
- Create: `backend/br8n/knowledge_graph/schema.py`
- Modify: `backend/br8n/knowledge_graph/models.py` (add `KGSchema` + supporting models if not already present)
...
Expected: FAIL — `ModuleNotFoundError: br8n.knowledge_graph.schema`.

**Step 3: Port the implementation**

Copy `delapan/knowledge_graph/schema.py` → `br8n/knowledge_graph/schema.py`. Change imports: `delapan.*` → `br8n.*`. Confirm `propose_schema` reads the LLM client the same way br8n's `exploration/` does (check `br8n/clients/` for the gateway client — match br8n's slug convention, not Delapan's). Add `KGSchema` + sub-models to `br8n/knowledge_graph/models.py` if not re-exported.
...
```bash
cd backend && ruff check br8n/knowledge_graph/schema.py br8n/knowledge_graph/models.py && ruff format br8n/knowledge_graph/schema.py
git add backend/br8n/knowledge_graph/schema.py backend/br8n/knowledge_graph/models.py backend/tests/knowledge_graph/test_schema.py
...
- Modify: `backend/br8n/store/base.py` (add abstract methods)
- Modify: `backend/br8n/store/supabase.py` (implement against `kg_schemas`)
- Modify: `backend/br8n/store/sqlite.py` (local-tier parity — a `kg_schemas` table)
- Test: `backend/tests/store/test_kg_schema_store.py`

Delapan's `service.py` hits `sb.table(...)` directly. br8n routes through `Store`, so the persistence becomes **store methods**: `get_kg_intent(kb_id) -> dict | None` (max version), `set_kg_intent(org_id, kb_id, schema) -> dict` (insert version+1), and reuse existing `kg_stats`/`list_kg_nodes` for the `emergent` half of `kg_schema_view`.
...
import pytest
from br8n.store.sqlite import SqliteStore  # adjust to actual class name
...
```bash
git add backend/br8n/store/base.py backend/br8n/store/supabase.py backend/br8n/store/sqlite.py backend/tests/store/test_kg_schema_store.py
...
- Create: `backend/br8n/knowledge_graph/extractor.py`
- Create: `backend/br8n/knowledge_graph/builder.py`
- Test: `backend/tests/knowledge_graph/test_builder.py`

`extractor.py` is pure (LLM extraction of nodes/edges from findings; schema-steered when intent given). `builder.py` orchestrates: load findings → extract → dedupe via `match_kg_nodes` → persist via `upsert_kg_nodes/edges` (br8n's store already has these). Adapt `build_graph(ctx, ...)` to take br8n's `TenantContext` and route persistence through `get_store(...)` instead of `service_client()` directly.
...
Copy both files, rewrite imports `delapan.*` → `br8n.*`, and replace direct `service_client()` persistence with `get_store(ctx.access_token, org_id=ctx.org_id)` calls (`upsert_kg_nodes`, `upsert_kg_edges`, `match_kg_nodes`, finding reads). Drop Delapan's `refresh_project_description` for v1 (YAGNI — note it as a follow-up). Keep `grounded_in` provenance (the 0006 columns exist).
...
```bash
git add backend/br8n/knowledge_graph/extractor.py backend/br8n/knowledge_graph/builder.py backend/tests/knowledge_graph/
...
async def test_kb_exists_false_for_unknown(monkeypatch, ...):
    ...
    out = await server.br8n_kb_exists("ghost-proj", "ghost-kb")
    assert out == {"exists": False, "project": "ghost-proj", "kb": "ghost-kb"}
```

**Step 2: Run → FAIL** (`br8n_kb_exists` undefined).
...
```bash
git add backend/br8n/interfaces/mcp/server.py backend/tests/interfaces/test_mcp_kb_exists.py
git commit -m "feat(mcp): br8n_kb_exists guard for first-run detection"
```
...
Add five tools mirroring Delapan, renamed `br8n_*`, routed through br8n's store/builder: `br8n_propose_kg_schema`, `br8n_set_kg_schema`, `br8n_get_kg_schema`, `br8n_build_graph`, `br8n_graph`, `br8n_kg_stats`. Reuse `KGSchema.model_validate` + `validate_schema` in `set` (Task 1), `set_kg_intent`/`get_kg_intent` (Task 2), `build_graph` (Task 3).

**Step 1: Failing test** — `br8n_set_kg_schema` rejects a malformed schema with `{ok: False, errors: [...]}` and accepts a clean one (mock the store's `set_kg_intent`).

**Step 2: Run → FAIL.**

**Step 3: Implement** the six tools, copying Delapan's docstrings (adjust tool names + br8n substrate language: "findings" stay findings). `propose` reads findings via the store; `build_graph` calls `builder.build_graph(ctx, ...)`.
...
```bash
git add backend/br8n/interfaces/mcp/server.py backend/tests/interfaces/test_mcp_kg_tools.py
...
Run: `python -c "from br8n.interfaces.mcp import server; print(sorted(t for t in dir(server) if t.startswith('br8n_')))"`
Expected: lists `br8n_build_graph`, `br8n_capture`, `br8n_explore`, `br8n_get_kg_schema`, `br8n_graph`, `br8n_kb_exists`, `br8n_kg_stats`, `br8n_propose_kg_schema`, `br8n_resume`, `br8n_set_kg_schema`, `br8n_activity`.
...
Near-verbatim port. Substrate adaptations: (1) MCP tool names `mcp__delapan__delapan_*` → `mcp__br8n__br8n_*`; (2) "the KB's findings" stays (br8n findings); (3) Stage 0 mode-select calls `br8n_get_kg_schema`; (4) Stage 5 build calls `br8n_build_graph(use_schema=True, rebuild=True)`; (5) drop references to delapan-only skills (`/delapan:build explore` → `/br8n:explore`; `/delapan:maintain` → omit or map to nearest br8n verb). Keep all five stages, the one-question-at-a-time `AskUserQuestion` discipline, and the Reconfigure flow.

**Verify:** Read the finished doc end-to-end; confirm every tool name resolves to a real `br8n_*` tool from Task 5, and no `delapan:` skill references remain.

```bash
grep -n "delapan" skills/_shared/kg-schema-wizard.md   # expect: no matches
git add skills/_shared/kg-schema-wizard.md
git commit -m "docs(skill): port 5-stage KG schema wizard to br8n substrate"
```
...
- **Phase A (local, no network):** crawl repo structure (tree, languages, entry points, manifests, build/test config), git (recent log, branches, contributors, churn hotspots), metadata (README, CLAUDE.md/AGENTS.md, docs/, license). Persist each as a finding via `mcp__br8n__br8n_capture` / the ingest path. Creating the KB is the concurrency lock.
- **Phase B (bounded web):** pick 2–4 high-value external facts from Phase A; one bounded `mcp__br8n__br8n_explore` pass with a small fixed budget; on failure, skip (local findings stand).
...
A SessionStart hook. Reads hook input (cwd) from stdin/env, resolves repo identity, calls `br8n_kb_exists`, and on first-run emits `additionalContext` injecting the directive. Make the logic importable (pure function `decide(cwd) -> str | None`) so it's testable without the hook harness.
...
- Modify: `backend/br8n/interfaces/mcp/server.py` (stamp `init_offered_at`)
...
Check whether `kbs` has a spare metadata column. If not, add `init_offered_at timestamptz` via a new migration `0007_kb_init_offered.sql`. Expose a tiny `br8n_mark_init_offered(project, kb, at)` tool (or fold into `set_kg_schema`'s success path). The wizard offer reads it: if set, don't re-offer.
...
```bash
git add .claude-plugin/plugin.json backend/br8n/interfaces/mcp/server.py supabase/migrations/0007_kb_init_offered.sql
...
**Step 1:** In a throwaway git repo with no br8n KB, launch a session. Observe: one-line announce; background subagent dispatched.

**Step 2:** Wait for completion. Inspect the KB: `br8n_kg_stats` / list findings → Phase-A findings present; Phase-B findings present (or cleanly skipped).
...
```bash
git add docs/plans/2026-06-01-br8n-first-run-init-design.md
...
- **Slug convention:** br8n's chat/agent path and pipeline path may use different model-slug formats (dots vs hyphens) like Delapan does. When porting `propose_schema`/`extractor` LLM calls, match **br8n's** existing `exploration/` client usage — do not copy Delapan's slugs.
- **Store routing is the #1 porting hazard.** Delapan calls `service_client()` inline; br8n *must* go through `get_store(...)`. Every `sb.table(...)` in the Delapan source becomes a store method call. Tasks 2 and 3 are where this bites.
- **Local tier parity:** br8n has a local/sqlite backend Delapan lacks. KG-schema persistence (Task 2) needs the sqlite path or the wizard breaks for local users. Builder persistence (Task 3) already has `upsert_kg_nodes/edges` in both backends — verify the sqlite versions exist and work.
