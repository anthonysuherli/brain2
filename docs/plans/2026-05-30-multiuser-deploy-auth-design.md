# br8n — Multi-user deploy + real auth (cloud tier)
...
| API key | **Retire `BR8N_API_KEY` as a user identity**; service-only | The phone sends a real per-user JWT; the static key identifies the deployment, not a user |
...
2. **The real blocker is a shared identity, not missing auth — and there's a
   ...
   phone would see the **server's** repos. Worse, br8n reads through the
...
- RLS **enabled** on `findings`, `kbs`, `projects`, `kg_nodes`, `kg_edges`,
  ...
  RLS is **not FORCED**, and br8n's cloud path uses the service-role key → RLS
...
The cloud tier forces `BR8N_BACKEND=cloud` (Supabase), which sidesteps the
local-tier loopback enforcement and the `~/.br8n/brain.db` path that would break
in a container.
...
## Deploy — Fly.io
- `Dockerfile` (python 3.11; `CMD ["uvicorn","br8n.api.main:app","--host",
  "0.0.0.0","--port","8002"]`, honoring `$PORT`).
- `fly.toml` (app name, region, `[http_service]` → internal port, `/health` check).
- Force `BR8N_BACKEND=cloud`; never the local tier on a hosted host.
