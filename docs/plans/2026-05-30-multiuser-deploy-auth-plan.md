...
**Files:**
- Modify: `backend/br8n/agent/state.py` (add `Principal` beside `TenantContext`)
- Test: `backend/tests/test_principal.py`

> `Principal` lives in `agent/state.py` (which imports nothing from `br8n`) so both `api/auth.py` and `interfaces/mcp/tenancy.py` can import it without the cycle `tenancy → api.auth → store → store.supabase → tenancy`.
...
**Step 5 — Commit:** `git add backend/br8n/agent/state.py backend/tests/test_principal.py && git commit -m "feat(auth): add Principal type for per-user tenancy"`
...
**Files:**
- Modify: `backend/br8n/api/auth.py`
- Modify: `backend/br8n/interfaces/mcp/tenancy.py` (add `_org_for_or_create`)
...
**Step 2 — Run, expect fail:** `AttributeError: module 'br8n.api.auth' has no attribute 'require_principal'`.
...
**Files:**
- Modify: `backend/br8n/store/__init__.py:57-70` (`get_store`)
- Modify: `backend/br8n/store/supabase.py:53-54, 220-280` (`__init__`, `resolve_project`, `list_projects`)
...
**Files:**
- Modify: `backend/br8n/interfaces/mcp/tenancy.py:79-123`
- Modify: `backend/br8n/interfaces/mcp/server.py` (call sites — `grep -n resolve_tenant`)
...
def test_local_resolve_ignores_principal(monkeypatch, tmp_path):
    ...
    import br8n.config as config; config.get_settings.cache_clear()
...
```
Add `from br8n.agent.state import Principal, TenantContext` (keep existing `TenantContext` import). In `interfaces/mcp/server.py`, build a principal from the configured login at each `resolve_tenant` call site:
```python
from br8n.agent.state import Principal
from br8n.interfaces.mcp.tenancy import _login, _org_for
_uid, _tok = _login()
...
- `backend/br8n/api/resume.py` (router dep + `resolve_tenant(principal, …)`; build stores with `org_id`)
- `backend/br8n/api/capture.py:57`
- `backend/br8n/api/explore.py:62`
- `backend/br8n/api/projects.py:17,38-41`
...
**Steps:** (1) extend `test_api_read_surfaces.py` with a `BR8N_BACKEND=cloud` + no-`SUPABASE_JWT_SECRET`/no-Bearer case asserting 401 on `/v1/projects`; and keep the existing local-tier 200 path. (2) run → fail. (3) apply the router/handler edits. (4) run → pass. (5) `git commit -m "feat(api): scope /v1 data endpoints to the authenticated principal"`.
...
**Files:**
- Modify: `backend/br8n/store/supabase.py:288-481` (`upsert_kg_nodes`, `upsert_kg_edges`, `match_kg_nodes`, `get_kg_subgraph`, `list_kg_nodes`, `kg_stats`)
- Modify: `backend/br8n/store/base.py` (update the protocol signatures if `org_id` becomes a param)
...
**Files:**
- Modify: `backend/br8n/knowledge_graph/activity.py` (`resolve_activity_target`, `_safe_target`, `query_activity`, `activity_rollup`, `activity_stats`, `schedule_activity_update`/`_run_activity_update`)
- Modify: `backend/br8n/api/activity.py` (router dep + handlers pass `principal`)
- Modify: `backend/br8n/api/capture.py` (pass `principal.org_id` + token into `schedule_activity_update`)
...
**Files:**
- Create: `backend/br8n/api/apple_auth.py`
- Modify: `backend/br8n/api/main.py` (include the router)
...
import br8n.api.apple_auth as mod
...
```
In `main.py`, `from br8n.api import apple_auth` and `app.include_router(apple_auth.router)` (no auth dependency on this router).
...
WORKDIR /app
ENV PYTHONUNBUFFERED=1 BR8N_BACKEND=cloud
COPY pyproject.toml ./
RUN pip install --no-cache-dir -e .
COPY br8n ./br8n
EXPOSE 8002
CMD ["sh", "-c", "uvicorn br8n.api.main:app --host 0.0.0.0 --port ${PORT:-8002}"]
```
`.dockerignore`: `.venv`, `.git`, `tests`, `__pycache__`, `*.pyc`, `../ios-app`, `../vscode-extension`. **Verify:** `docker build -t br8n backend && docker run -e SUPABASE_URL=… -e PORT=8080 -p 8080:8080 br8n` → `curl localhost:8080/health` returns `{"status":"ok"}`. Commit.
...
- `fly launch --no-deploy` (or `fly apps create br8n-api`).
...
- (Optional) attach the `api.br8n.dev` domain: `fly certs add api.br8n.dev` + DNS.

---

## Phase D — Supabase Apple provider (dashboard, one-time)
Checklist (not code; document completion in the PR):
1. Apple Developer portal: App ID has "Sign in with Apple" capability; note the **bundle id** (e.g. `com.br8n.app`).
...
**Files:** `ios-app/Br8n/Auth/AppleSignIn.swift`. Generate a cryptographically-random `rawNonce`; set `request.nonce = sha256(rawNonce)`; retain `rawNonce` to send to the backend. Add a `sha256(_:)` helper (CryptoKit). Manual build-check in Xcode (`xcodegen generate && open Br8n.xcodeproj`).

### Task E2: Call `/v1/auth/apple` + store session
**Files:** `ios-app/Br8n/Auth/AuthStore.swift`, `Networking/APIClient.swift`, `Auth/Keychain.swift`. On `didCompleteWithAuthorization`, POST `{ identity_token: identityToken, nonce: rawNonce }` to `/v1/auth/apple`; decode `SessionResponse`; persist `access_token` + `refresh_token` + `expires_at` in Keychain (service `dev.br8n.app`); set signed-in state.
...
**Files:** `ios-app/Br8n/Config.swift` (base URL → `https://api.br8n.dev` or the Fly URL), `Views/SignInView.swift` (remove the manual base-URL/token `ConnectionSheet`).
...
- `CLAUDE.md`: update the cloud-auth notes (BR8N_API_KEY is now service-only; cloud tenancy is JWT-driven; add `/v1/auth/apple` + `/v1/auth/refresh` to the API table) — do this as the last commit.
