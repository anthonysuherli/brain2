...
**Goal:** Rewrite `scripts/brain2-statusline.py` from a cross-branch portfolio into a single-focus two-line resume cue: hypothesis + state-adaptive trust bar (drift verdict, age, churn glyphs, action hint).

**Architecture:** Pure-function core (`parse_diff_stat_block`, `compute_drift`, `classify`, `render_line2`) unit-tested in isolation; thin I/O shell (`main`) wires git, SQLite/Supabase fetch, and the never-fail wrapper. Old portfolio renderer preserved behind `BRAIN2_STATUSLINE=portfolio`.
...
- Modify: `scripts/brain2-statusline.py`
...
**Step 3: Implement `parse_diff_stat_block` in `scripts/brain2-statusline.py`**
...
```bash
git add scripts/test_statusline.py scripts/brain2-statusline.py
...
- Modify: `scripts/brain2-statusline.py` (add functions)
...
**Step 3: Implement `compute_drift` and `classify` in `scripts/brain2-statusline.py`**
...
def test_render_line1_with_hypothesis():
    line = _strip(sl.render_line1("brain2", "dev", "fix the thing", width=80))
...
def test_render_line1_no_hypothesis():
    line = _strip(sl.render_line1("brain2", "dev", None, width=80))
    assert "▶ dev" in line
...
def test_render_line1_truncates_hypothesis():
    long_hyp = "x" * 100
    line = _strip(sl.render_line1("brain2", "dev", long_hyp, width=60))
    assert "…" in line
...
Add after `classify` in `scripts/brain2-statusline.py`:
...
- Modify: `scripts/brain2-statusline.py` (add helpers)
...
**Step 3: Implement the helpers in `scripts/brain2-statusline.py`**
...
Rewrites `main()` to use all the new functions. The old portfolio renderer is preserved behind `BRAIN2_STATUSLINE=portfolio`. Adds a never-fail integration test that pipes synthetic stdin and checks for exit 0 with two output lines.

**Files:**
- Modify: `scripts/brain2-statusline.py` (rewrite `main`)
- Modify: `scripts/test_statusline.py` (append never-fail + integration tests)

**Step 1: Replace `main()` in `scripts/brain2-statusline.py`**
...
def _portfolio_main() -> None:
    """Legacy cross-branch portfolio view (BRAIN2_STATUSLINE=portfolio)."""
...
def _run_statusline(stdin_json: str, env: dict | None = None) -> tuple[int, str]:
    ...
    import pathlib
    script = str(pathlib.Path(__file__).parent / "brain2-statusline.py")
...
def test_never_fails_non_git_cwd():
    code, _ = _run_statusline(
        '{"cwd": "/tmp"}',
        env={"BRAIN2_BACKEND": "local", "BRAIN2_DB_PATH": "/dev/null"},
    )
    assert code == 0
...
def test_never_fails_git_absent(tmp_path):
    ...
    code, _ = _run_statusline(
        ...
        env={"PATH": "/dev/null", "BRAIN2_BACKEND": "local"},
    )
    assert code == 0


def test_smoke_local_tier_no_capture(tmp_path):
    """Smoke: run in a real git repo with no brain2 DB → exits 0, shows branch."""
    repo = _make_repo(tmp_path)
    db = str(tmp_path / "empty.db")
    code, out = _run_statusline(
        json.dumps({"cwd": str(repo)}),
        env={"BRAIN2_BACKEND": "local", "BRAIN2_DB_PATH": db},
    )
    assert code == 0
...
```bash
echo '{"cwd": "'$(pwd)'"}' | BRAIN2_BACKEND=local python3 scripts/brain2-statusline.py
```

Expected: two lines — line 1 with `🧠 brain2 ▶ feat/statusline-resume-cue`, line 2 with `⚡ no capture yet · /brain2:capture to anchor` (since the test worktree has no snapshots).

**Step 5: Commit**

```bash
git add scripts/brain2-statusline.py scripts/test_statusline.py
git commit -m "feat(statusline): rewrite main() as two-line resume cue; keep portfolio behind BRAIN2_STATUSLINE=portfolio"
```
...
**Files:**
- Modify: `scripts/brain2-statusline.py` (docstring only)
...
Expected: `"command": "python3 /Users/suherli/Repositories/brain2/scripts/brain2-statusline.py"` — no change needed.
...
```bash
git add scripts/brain2-statusline.py
