...
Every brain2 user gets a default **activity knowledge graph**: a per-user, cross-repo
...
4. **Surfaces:** `brain2_activity` MCP tool + cross-repo resume card + `/v1/activity`
...
New module `backend/brain2/knowledge_graph/activity.py`:
...
- `interfaces/mcp/server.py::brain2_capture` — after persist.
...
**Config.**
- `BRAIN2_ACTIVITY_KG=1` (default on) — master switch; off = capture behaves as today.
- `BRAIN2_ACTIVITY_LLM=1` (default on) — gates only the hypothesis→Task LLM pass;
  off = deterministic-only (free, instant).
- `BRAIN2_KG_DEDUPE_THRESHOLD` (default ~0.92) — node-merge cosine threshold.
...
- `interfaces/mcp/server.py::brain2_activity` tool
