# br8n as a Personal Knowledge Engine — Synthesis/Distillation Design
...
br8n today is a **developer capture/resume engine**: per-repo+branch sessions
produce resume cards, and a web gap-fill pipeline (explore) seeds KBs. This design
repositions br8n as a **personal knowledge engine**: one per-user graph that
...
The wedge is **synthesis/distillation**: br8n doesn't just store what you read
...
- **On explore completion** — after `run_exploration` persists findings
  ...
  immediately.
- **Explicit** — `/br8n:distill [topic]` / `br8n_distill` forces a pass over a
  topic or the whole recent neighborhood (the synchronous, user-driven path).

Master gate `BR8N_DISTILL_KG` (default on); LLM sub-gate `BR8N_DISTILL_LLM`
(default on; off = no synthesis, pipeline no-ops). Mirrors
`BR8N_ACTIVITY_KG` / `BR8N_ACTIVITY_LLM`.
...
**New endpoint** `GET /v1/context` and MCP tool `br8n_context`:
...
repos when it bears on what you're doing now. `/br8n:resume` and `/br8n:search`
...
| Surface | Change |
|---|---|
| `br8n_context` (MCP) + `GET /v1/context` | **New** — the portable grounding packet (§6) |
| `br8n_distill` (MCP) + `POST /v1/distill` | **New** — force a distillation pass |
| `/br8n:distill` skill | **New** — `skills/distill/SKILL.md` |
| `/br8n:resume`, `/br8n:search` | Re-point at the context assembler (concepts lead) |
...
`/v1/context` + `br8n_context`; re-point resume/search. *Done when:*
`br8n_context` returns concepts-over-evidence and surfaces a cross-KB concept when
relevant.

**Phase D — Explicit surface.** `/br8n:distill` skill + `br8n_distill`.
...
- **Distillation quality is an LLM-judgment problem.** Without firm critic gating
  ...
  multi-source concepts, and keep `BR8N_DISTILL_LLM` as a kill switch.
