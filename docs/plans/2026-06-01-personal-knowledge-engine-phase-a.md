...
**Files:**
- Modify: `brain2/store/base.py` (KG section, after `upsert_kg_edges` ~line 118)

**Step 1: Add the protocol method**

In `brain2/store/base.py`, inside `class Store(Protocol)`, after `upsert_kg_edges`:
...
**Files:**
- Modify: `brain2/store/sqlite.py` (after `_merge_kg_node`, ~line 548)
...
In `brain2/store/sqlite.py`, after `_merge_kg_node`:
...
```bash
git add brain2/store/sqlite.py tests/test_kg_store_sqlite.py
...
**Files:**
- Modify: `brain2/store/supabase.py` (KG section, near `upsert_kg_nodes`)
...
**Step 3: Implement** in `brain2/store/supabase.py`, mirroring `upsert_kg_nodes`'s table/embedding handling:
...
```bash
git add brain2/store/supabase.py tests/test_store_supabase.py
...
**Files:**
- Modify: `brain2/config.py` (add class after `ActivityConfig` ~line 170; wire into `Config` ~line 201)
...
In `brain2/config.py`, after `ActivityConfig`:
...
```bash
git add brain2/config.py tests/test_config_concept.py
...
**Files:**
- Create: `brain2/knowledge_graph/distill.py`
...
Expected: FAIL — module `brain2.knowledge_graph.distill` does not exist.

**Step 3: Implement** `brain2/knowledge_graph/distill.py` (models + synthesize only; reconcile/persist land in Task 6):
...
```bash
git add brain2/knowledge_graph/distill.py tests/test_distill_synthesize.py
...
**Files:**
- Modify: `brain2/knowledge_graph/distill.py`
...
```python
from brain2.config import get_config
from brain2.knowledge_graph.distill import ConceptCandidate, reconcile_action
...
**Step 3: Implement** — append to `brain2/knowledge_graph/distill.py`:
...
```bash
git add brain2/knowledge_graph/distill.py tests/test_distill_reconcile.py
...
```bash
git add brain2/knowledge_graph/distill.py tests/test_distill_flow.py
...
from brain2.exploration.evaluator import evaluate_findings  # reuse the critic
...
- **Phase B** — triggers: chain `distill_kb` off `api/capture.py` (after
  ...
  gated (`BRAIN2_DISTILL_KG`/`_LLM`), fire-and-forget. Add the critic gate here
